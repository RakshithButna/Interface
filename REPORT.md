...
- **Live verification of every provider.** The `LlmProvider` seam has two implementations: an OpenAI-compatible client (Groq, OpenAI, OpenRouter, Together, vLLM, Ollama) and a separate Anthropic client, since the Messages API differs structurally — top-level `system`, `input_schema` tools, `tool_use` content blocks rather than a `tool_calls` array, tool results as user-message blocks, and no sampling parameters at all (`temperature` returns a 400 on current models). The factory auto-detects from whichever key is present. Both wire formats are covered by tests against a mock server, which is where translation bugs actually live; I have only exercised Groq against the live API, and I'd rather say that plainly than imply broader verification than I did.
